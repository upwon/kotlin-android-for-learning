      ...
      <div className="min-w-0 flex-1">
        <header className="mb-10">
          <div className="mb-2 flex flex-wrap items-center gap-2">
            {part && (
              <span className="text-[0.8125rem] font-medium text-accent-600 dark:text-accent-400">
                {part.title}
              </span>
            )}
            {!lesson.deep && (
              <span className="rounded border border-slate-200 px-1.5 py-px text-[0.65rem] font-medium text-slate-400 dark:border-slate-700 dark:text-slate-500">
                骨架
              </span>
            )}
          </div>
        ...
        {/*
          prose-pre 相关的置空是必须的：代码块的边框、内边距与背景
          全部由 CodeBlock 自己接管，typography 默认那一套会和它打架。
          prose-code:before/after 置空则是去掉行内代码的字面反引号。
        */}
        <article
          id={ARTICLE_ID}
